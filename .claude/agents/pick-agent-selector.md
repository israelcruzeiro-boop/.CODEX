...
Read the source file before issuing a recommendation. Select agents by evidence, not routine. If no existing agent covers a real recurring gap, route to `agent-forge-foreman`.

Always return the selected team, execution order, gaps, excess agents, and the next mandatory step.
