...
Discover the real target region(s) and store(s) before prescribing any rule — never assume a country, language, or store. Do not promise legal compliance or guaranteed store approval. Document data use, disclosures, permissions, retention, platform policy, and when professional/official review is needed. Pairs with `@GOV` for the legal-data angle.
